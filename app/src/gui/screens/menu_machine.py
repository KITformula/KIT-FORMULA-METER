...
# ★追加: タイヤ選択画面
class TireSelectScreen(QWidget):
    tireSetChanged = pyqtSignal(str)
    requestBack = pyqtSignal()

    def __init__(self):
        super().__init__()
        self.layout = QVBoxLayout()
        self.layout.addWidget(QLabel("TIRE SELECT", alignment=Qt.AlignCenter, styleSheet="font-size: 32px; font-weight: bold; color: #ADFF2F;"))
        
        self.list = QListWidget()
        self.list.setStyleSheet("""
            QListWidget { font-size: 40px; background-color: #222; color: white; } 
            QListWidget::item { padding: 20px; }
            QListWidget::item:selected { background-color: #556B2F; border: 2px solid #ADFF2F; }
        """)
        
        # 選択肢
        self.tires = ["Dry 1", "Dry 2", "Dry 3", "Dry 4", "Dry 5", "Wet 1", "Wet 2"]
        self.list.addItems(self.tires)
        self.list.addItem("<< BACK") # 最後の項目
        
        self.list.setCurrentRow(0)
        self.layout.addWidget(self.list)
        self.layout.addWidget(QLabel("Push: SET & BACK", alignment=Qt.AlignCenter, styleSheet="font-size: 20px; color: #AAA;"))
        self.setLayout(self.layout)
        p = self.palette(); p.setColor(self.backgroundRole(), QColor("#333")); self.setPalette(p); self.setAutoFillBackground(True)
    ...
    def handle_input(self, input_type: str) -> bool:
        ...
        if input_type == "CW": 
            self.list.setCurrentRow(0 if row >= count-1 else row + 1)
            return True
        elif input_type == "CCW": 
            self.list.setCurrentRow(count-1 if row <= 0 else row - 1)
            return True
            ...
            if row == count - 1:
                ...
                selected_tire = self.tires[row]
                self.tireSetChanged.emit(selected_tire)
                self.requestBack.emit()
            return True
        return False